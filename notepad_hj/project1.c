...
U8 o_sw, n_sw;                         
U8 EEPROM_READ_DATA_DB[100];

const U8 CHECK_SUM1=0x00;    // 2013.10.29 VERSION		
const U8 CHECK_SUM2=0x00;    // 2013.10.29 VERSION		
const U8 EEPROM_DB[100]={     // 100개 배열
	0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,0x1A,0x54,
	0x00,0x00,0x00,0x00,0x00,0x00,0x1B,0x12,0x01,0x03,
	0x80,0x34,0x20,0x78,0x2A,0xC9,0x05,0xA3,0x57,0x4B,
	0x9C,0x25,0x12,0x50,0x54,0x21,0x08,0x00,0xA9,0x40,
	0x81,0x80,0x8B,0xC0,0x95,0x00,0xB3,0x00,0xD1,0xC0,
	0xA9,0xC0,0x01,0x01,0x00,0x00,0x00,0xFC,0x00,0x20,
	0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,
	0x20,0x20,0xFC,0x1C,0x80,0x18,0x71,0x1C,0x16,0x20,
	0x58,0x2C,0x25,0x00,0x06,0x44,0x21,0x00,0x00,0x9E,
	0x00,0x00,0x00,0xFD,0x00,0x3C,0x3D,0x1F,0x4D,0x11};
...
void Byte_write_100(void);
void Byte_read_100(void);
...
void main(void){
	...
	o_sw = PINE & 0b11110000;      // SW1=PE4 상태값	
    while(1){
        n_sw = PINE  & 0b11110000;    
        if(o_sw ==0xF0 && n_sw == 0xE0) Byte_write(); 
		else if(o_sw ==0xF0 && n_sw == 0xD0) Random_read();
		else if(o_sw ==0xF0 && n_sw == 0xB0) Random_read();
		else if(o_sw ==0xF0 && n_sw == 0x70) Random_read();
		o_sw = n_sw;         
    }//end of while
}//end of main
...
void Byte_write_100(void) {   // Bytewrite 함수 
    IIC_START();              // start
	int i;
	...
	Process_8data(IIC_ADD); // Address 8bit
    ACK_read();              // ack
	
	for(i=0; i<100, i++)
	{
     	Process_8data(EEPROM_DB[i]);  // data 8bit
        ACK_read();              // ack
	}
        IIC_STOP();           // stop  
}//end of Byte_write   

void Byte_read_100(void) {   // Bytewrite 함수 
    IIC_START();              // start
	int j;
	...
	for(j=0; j<100, j++)
	{
     	Process_8data(EEPROM_DB[j]);  // data 8bit
        ACK_read();              // ack
	}
        IIC_STOP();           // stop  
}//end of Byte_read
